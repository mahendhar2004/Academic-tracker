...
const AddExpenditureModal = ({ isOpen, onClose, onSave }) => {
    ...
    const handleSubmit = (e) => {
        ...
        if (expense.title.trim() && !isNaN(amountNum) && amountNum > 0 && expense.date) {
            onSave({ ...expense, amount: amountNum, date: Timestamp.fromDate(new Date(expense.date)) });
            onClose();
        }
    };

    return (
        <GlassyModal isOpen={isOpen} onClose={onClose} title="Add New Expense">
            <form onSubmit={handleSubmit} className="space-y-4 w-80 md:w-96">
                ...
                <div className="flex gap-4">
                    <div className="w-1/2">
                        ...
                        <input id="expenseAmount" type="number" value={expense.amount} onChange={(e) => handleChange('amount', e.target.value)} placeholder="e.g., 500" className="w-full bg-black/20 border border-white/20 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-cyan-400" required min="1" step="any" />
                    </div>
                    <div className="w-1/2">
                ...
                <motion.button whileTap={{ scale: 0.95 }} type="submit" className="w-full bg-cyan-500/50 hover:bg-cyan-500/80 border border-cyan-400/50 text-white font-bold py-3 px-4 rounded-lg transition-colors !mt-6">Add Expense</motion.button>
            </form>
        </GlassyModal>
    );
};
